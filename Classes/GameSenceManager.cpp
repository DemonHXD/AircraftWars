...
Scene* GameSenceManager::createScene() {
	...
	auto layer = GameSenceManager::create();
...
bool GameSenceManager::init() {
	...
	//开启创建道具调度器
	schedule(schedule_selector(GameSenceManager::createPropSchedule), 25, -1, 0);
...
/*
	自定义生产敌机的调度器
*/
void GameSenceManager::createEnemy(float dt) {
	float x;
...
/*
	英雄子弹与敌机碰撞检测
*/
void GameSenceManager::collisionBulletAndEenmy() {
...
/*
	英雄与道具碰撞检测
*/
void GameSenceManager::collisionHeroAndProp() {
	Hero* hero = (Hero*)this->getChildByTag(HERO);
...
/*
	判断英雄与敌机子弹的碰撞
*/
void GameSenceManager::collisionHeroAndEenmyBullet() {
	Hero* hero = (Hero*)this->getChildByTag(HERO);
